<div class="cart-summary">
                            <h4>{{App::getLocale() == 'ar' ? 'ملخص الطلب ' : 'order summary'}}</h4>
                            <div class="total-summary">
                                <p>{{App::getLocale() == 'ar' ? "سعر الطلب  ": "subtotal"}}: <span>EGP{{$subtotal}}</span></p>
                                <p>{{App::getLocale() == 'ar' ? " الشحن  ": "shipping"}}: <span>EGP {{$shipping}}</span></p>
                                <p>{{App::getLocale() == 'ar' ? " الضرائب": "taxes"}}: <span>EGP {{$taxes}}</span></p>
                                <p>{{App::getLocale() == 'ar' ? " الخصم": "discount"}}: <span>EGP {{$discount}}</span></p><hr>
                                <p class="total">{{App::getLocale() == 'ar' ? " السعر الكلي ": "total"}}<span>EGP {{$total}}</span></p>
                                <!-- <a href="checkout.html" class="btn btn-block btn-summary">Proceed to checkout</a> -->
                            </div>
                            <div class="coupon">
                                <div class="acco-header">
                                    <h4>{{App::getLocale() == 'ar' ? " الخصم ": "Purchase Coupon"}}</h4>
                                </div>
                                <div class="acco-body">
                           
                                    <input type="text" id="coupon">
                                    <a href="#" class="btn btn-block btn-summary">{{App::getLocale() == 'ar' ? " اضافة كود الخصم  ": "Apply Coupon"}}</a>
                                    <p>{{App::getLocale() == 'ar' ? "يمكنك اضافة كود خصم واحد فقط":"You can apply only one coupon" }}</p>
                                </div>
                            </div>

                        </div>