@extends('layoutDashboard.app',['title'=>' غلق الويب سايت '])
@section('style')
...
@component('components.panel',['subTitle'=>' غلق الويب سايت '])  
<div class="container-fluid">
        <div class="row">
          <div class="col-md-12 ">
    <form id="closeForm" action="{{route('appSetting.close.edit')}}" method="post" enctype="multipart/form-data" class="settings main-box">
         @csrf
            <div class="group">
              <div class="title">فتح المتجر    </div>
              <div class="switch-field open-mode">
                  
                <input type="radio" id="mode-yes" name="Close" value="yes" {{$appSetting->Close  == 'yes' ? 'checked' : ' '}} >
                <label for="mode-yes">نعم </label>
                <input type="radio" id="mode-no" name="Close" value="no" {{$appSetting->Close  == 'no' ? 'checked' : ' '}}>
                <label for="mode-no">لا </label>
              </div>
            </div>
            <input id="closeType" type="text" hidden name="closetype">
            
            @if($appSetting->Close  == 'no')
            <div class="group mode-templates " >
              @else 
                  <div class="group mode-templates hidden-content" >
                @endif
           
              <div class="{{$appSetting->CloseType == 1 ?   'template selected' : 'template '  }}" data-id=1>
                <i class="fas fa-socks"></i>
                <p class="message">المتجر مغلق للصيانة ، تفضل بزيارتنا قريبا</p>
              </div>
              <div class="{{$appSetting->CloseType !== 1 ?   'template selected' : 'template '  }}" data-id=2>
...
<!--          <form role="form" action="{{route('appSetting.close.edit')}}" method="post" enctype="multipart/form-data">-->
<!--          @csrf-->
<!--                <div class="card-body">-->
                  
<!--                <div class="checkbox">-->
<!--  <label>-->
<!--      @if($appSetting->Close  == '1')-->
