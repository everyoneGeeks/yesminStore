...
@section('content')
@component('components.panel',['subTitle'=>' بيانات المنتج المرتجع  '])
...
<div class="container-fluid">
  ...
  @component('components.panel',['subTitle'=>'منتجات الطلب  '])
...
@component('components.panel',['subTitle'=>' المنتج المرتجع '])
          ...
          <div class="col-md-12 text-center">
            ...
            <div class="card card-primary card-outline">
              <div class="card-body box-profile">
                <div class="text-center">
                  <img class="img-fluid "
                   width="150px" height="150px" 
                    ...
                    src="{{asset($Complains->product->main_image)}}"
                ...
                <ul class="list-group list-group-unbordered mb-3">
                     <li class="list-group-item">
                    <span><a href="/admin/product/info/{{$product->product->id}}"> {{$Complains->product->name_ar}} </a> </span> <b class="float-right">الاسم    </b>
                  </li>

                  <li class="list-group-item">
                  <b class="float-right">الوصف    </b>
                    <span>{{$Complains->product->description_ar}}</span> 
                  </li>

                  @if($Complains->character !==null)
                  <li class="list-group-item">
               <span>{{$Complains->character->name_ar}}  </span> <b class="float-right">  الشخصية   </b>
                  </li>
                  @endif

                  @if($Complains->occasion !== null ) 
                  <li class="list-group-item">
                <span>{{$Complains->occasion->name_ar}}  </span> <b class="float-right">  المناسبة   </b>
                  </li>
                  @endif


                  @if($Complains->party_theme !==null )
                  <li class="list-group-item"><span>{{$Complains->party_theme->name_ar}}  </span> <b class="float-right">  نوع الحفل    </b>
                  </li>
                  @endif


                  @if($Complains->size !==null)
                  <li class="list-group-item">
                     <span>{{$Complains->size->name_ar}}  </span> <b class="float-right">  الحجم    </b>
                  </li>
                  @endif
                  ...
                  <li class="list-group-item">
                    <span>{{$Complains->price}}</span> <b class="float-right">  السعر   </b>
                  </li>
                  <li class="list-group-item">
                    <span>{{$Complains->amount}}</span> <b class="float-right">  الكمية    </b>
                  </li>

                  <li class="list-group-item">
                    <span>{{$Complains->discount}}</span> <b class="float-right">  نسبة الخصم  </b>
                  </li>


@if($Complains->personalize)
<hr>
<p> اضافة الطابع الشخصي </p>
                  <li class="list-group-item">
                    <span>{{$Complains->child_name}}</span> <b class="float-right">     </b>
                  </li>
                  <li class="list-group-item">
                    <span>{{$Complains->child_age}}</span> <b class="float-right">  الكمية    </b>
                  </li>
